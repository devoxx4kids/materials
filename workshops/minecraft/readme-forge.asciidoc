...
. Double click on `forge` folder in `Package Explorer` on the left side. This is where all the source code exists and we'll modify it later. Double clic on `forge` folder in `Gradle Task`, double click on `forgegradle runs`, and select `runClient`.
