...
. Make a new directory by giving the command `mkdir forge`
. Change to the directory `cd forge`
. In File Explorer, double click on the Forge 1.19 zip file downloaded earlier and extract all the contents.
. Copy the contents from this extracted folder to `forge` directory using the command `cp -r ~/Downloads/forge-1.19-41.1.0-mdk/* .`.
