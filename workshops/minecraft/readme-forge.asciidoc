...
**Instructions:** In your new package, `org.devoxx4kids.forge.mods`, make a new class called `ChatItems`. To do this, right click on the package `org.devoxx4kids.forge.mods` and select `New` > `Class` then fill the name property with `ChatItems` and clico on the Finish Button. Replace its code with <<Chat_Items_Code>>. 
