...
**Purpose:** The first mod wil add items to player's inventor when a certain word or phrase is spelled in a chat message.

**Instructions:** THe `ssrc/main/java` folder already has a `com.example.examplemod` package. We need to make a new class called `ChatItems`. To do this, right click on the package `com.example.examplemod` and select `New` > `Class` then fill the name property with `ChatItems` and click on the `Finish` Button. Replace its code with <<Chat_Items_Code>>. 
...
Close Minecraft game window and restart it by clicking on the "`Run Client`" button on the Eclipse menu bar.

=== Different Items

This item produces different items for different chat text. For example, `potato` chat text will yield a potato in the inventory and `diamond` chat text will yield a diamond in the inventory.
...
This mod will produce multiple items for a chat text. Instead of only producing potato and diamond at one time, it can product potato and diamond, both at the same time, on the same text.
...
After doing this, press `Control` + `Shift` + `O` on a Windows computer or `Cmd` + `Shift` + `O` on a Mac computer to import one class and fix the error. In the minecraft game window, give yourself a sponge using the command  `/give Dev minecraft:sponge`. Select slot from the inventory?? in which the sponge is placed, double click in the world to place down the sponge, and this will spawn a Squid.

=== Change offset of entities

Spawn ender dragon 2 blocks above the location where the dragon egg is placed.
